...
from ingenious.chat.domain.models import ChatResponse
...
class TestChatAPIIntegration:
    ...
    async def test_chat_endpoint_success(
        self, async_client, mock_chat_application_service
    ):
        ...
        with patch(
            "ingenious.dependencies.get_chat_service",
            return_value=mock_chat_application_service,
        ):
            with patch(
                "ingenious.dependencies.get_security_service", return_value=None
            ):
                # Act
                response = await async_client.post(
                    "/api/v1/chat", json=chat_request, auth=("testuser", "testpass")
                )
    ...
    async def test_chat_endpoint_validation_error(self, async_client):
        ...
        with patch("ingenious.dependencies.get_security_service", return_value=None):
            # Act
            response = await async_client.post(
                "/api/v1/chat", json=invalid_request, auth=("testuser", "testpass")
            )
    ...
    async def test_chat_endpoint_content_filter_error(
        self, async_client, mock_chat_application_service
    ):
        ...
        mock_chat_application_service.process_chat.side_effect = ContentFilterError(
            "Content filtered"
        )
    ...
    async def test_chat_endpoint_token_limit_error(
        self, async_client, mock_chat_application_service
    ):
        ...
        mock_chat_application_service.process_chat.side_effect = (
    ...
    async def test_chat_endpoint_server_error(
        self, async_client, mock_chat_application_service
    ):
        """Test chat endpoint with internal server error."""
        # Arrange
        mock_chat_application_service.process_chat.side_effect = Exception(
            "Internal error"
        )

        chat_request = {"user_prompt": "Hello!", "conversation_flow": "general"}
    ...
    async def test_get_conversation_endpoint_success(self, async_client):
        ...
        with patch(
            "ingenious.dependencies.get_chat_service", return_value=mock_chat_service
        ):
            with patch(
                "ingenious.dependencies.get_security_service", return_value=None
            ):
                ...
                response = await async_client.get(
                    "/api/v1/conversations/thread-123", auth=("testuser", "testpass")
                )
    ...
    async def test_authentication_required(self, async_client):
        ...
        response = await async_client.put(
            "/api/v1/messages/msg-456/feedback", json={"feedback_type": "thumbs_up"}
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    async def test_cors_headers(self, async_client):
        ...
        # Arrange
        chat_request = {"user_prompt": "Test CORS", "conversation_flow": "general"}
    ...
    async def test_request_size_limits(self, async_client):
        ...
        chat_request = {"user_prompt": large_prompt, "conversation_flow": "general"}
    ...
    async def test_concurrent_requests(self, async_client):
        ...
        async def make_request(prompt_id):
            ...
            return await async_client.post(
                "/api/v1/chat", json=request, auth=("testuser", "testpass")
            )
