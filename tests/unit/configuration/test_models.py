...
class TestConfigurationItem:
    ...
    def test_configuration_item_creation_basic(self):
        ...
        item = ConfigurationItem(
            key="database.host", value="localhost", description="Database host address"
        )
    ...
    def test_configuration_item_serialization(self):
        ...
        # Arrange
        item = ConfigurationItem(key="test.key", value="test.value", is_secret=True)
...
class TestMinimalConfig:
    ...
    def test_minimal_config_serialization(self):
        ...
        original_config = MinimalConfig(
            project_name="serialization-test", environment="test", log_level="DEBUG"
        )
...
class TestConfigurationModelsIntegration:
    ...
    def test_complete_configuration_setup(self):
        ...
        auth_config = AuthenticationConfig(
            enabled=True, provider="oauth2", token_expiry=7200
        )

        llm_config = LLMConfig(
            provider="azure", model="gpt-4", temperature=0.8, max_tokens=1500
        )
    ...
    def test_configuration_defaults_consistency(self):
        ...
        assert (
            app_config.environment == minimal_config.environment
        )  # Both default to "development"
        assert (
            app_config.log_level == minimal_config.log_level
