...
class TestChatApplicationService:
    ...
    async def test_process_chat_success(self, chat_app_service, mock_chat_service, 
                                       valid_chat_request, mock_chat_response):
        ...
        mock_chat_service.process_chat_request.assert_called_once_with(valid_chat_request)
    ...
    async def test_process_chat_validation_missing_conversation_flow(self, chat_app_service):
    ...
    async def test_process_chat_domain_service_exception(self, chat_app_service, 
                                                        mock_chat_service, valid_chat_request):
    ...
    async def test_process_chat_specific_llm_exception(self, chat_app_service, 
                                                      mock_chat_service, valid_chat_request):
        ...
        mock_chat_service.process_chat_request.side_effect = TokenLimitExceededError("Token limit exceeded")
    ...
    def test_enhance_response_basic(self, chat_app_service, mock_chat_response):
        ...
        # Assert
        assert enhanced_response == mock_chat_response  # Currently just returns the same response
    ...
    def test_handle_error_with_specific_error_type(self, chat_app_service, valid_chat_request):
    ...
    async def test_process_chat_end_to_end_success(self, chat_app_service, 
                                                  mock_chat_service, valid_chat_request):
    ...
    async def test_process_chat_with_long_prompt(self, chat_app_service, 
                                                mock_chat_service, mock_chat_response):
    ...
    async def test_process_chat_with_special_characters(self, chat_app_service, 
                                                       mock_chat_service, mock_chat_response):
        """Test chat processing with special characters in prompt."""
        # Arrange
        request = ChatRequest(
            user_prompt="Hello! @#$%^&*()_+ 🚀 ñáéíóú",
            conversation_flow="general"
        )
        mock_chat_service.process_chat_request.return_value = mock_chat_response
