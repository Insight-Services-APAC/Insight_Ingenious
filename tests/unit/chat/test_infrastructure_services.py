...
from ingenious.chat.domain.entities import ChatSession
from ingenious.chat.domain.models import ChatRequest, ChatResponse
from ingenious.chat.infrastructure.services import (
...
class TestModernChatService:
    ...
    async def test_process_chat_request_success(
        self, chat_service, mock_llm_service, valid_chat_request
    ):
    ...
    async def test_process_chat_request_empty_prompt(self, chat_service):
        ...
        # Arrange
        request = ChatRequest(
            user_prompt="", conversation_flow="general", user_id="user-123"
        )
    ...
    async def test_process_chat_request_llm_service_exception(
        self, chat_service, mock_llm_service, valid_chat_request
    ):
    ...
    async def test_process_chat_request_with_options(
    ...
    async def test_process_chat_request_no_llm_response(
        self, chat_service, mock_llm_service, valid_chat_request
    ):
    ...
    async def test_process_chat_request_partial_llm_response(
        self, chat_service, mock_llm_service, valid_chat_request
    ):
    ...
    async def test_process_chat_request_different_conversation_flows(
        self, chat_service, mock_llm_service
    ):
...
class TestDefaultConversationService:
    ...
    async def test_continue_conversation_nonexistent_session(
        ...
        # Act & Assert
        with pytest.raises(ValueError) as exc_info:
            await conversation_service.continue_conversation(
    ...
    async def test_conversation_flow_with_multiple_messages(self, conversation_service):
        ...
        response1 = await conversation_service.continue_conversation(
            session_id, "First message"
        )
        response2 = await conversation_service.continue_conversation(
            session_id, "Second message"
        )
        response3 = await conversation_service.continue_conversation(
            session_id, "Third message"
        )
...
class TestChatInfrastructureIntegration:
    ...
    async def test_error_handling_across_services(self, integrated_services):
        ...
        assert (
            "Error processing request" in response.response
        )  # Chat service handles error
