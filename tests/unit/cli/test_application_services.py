...
from ingenious.cli.domain.services import IProjectService, IServerService, ITemplateService
...
class TestCLIApplicationService:
    ...
    @pytest.fixture
    def cli_service(self, mock_project_service, mock_server_service, mock_template_service):
    ...
    def test_create_project_success(self, cli_service, mock_project_service, mock_template_service):
    ...
    def test_create_project_with_profile(self, cli_service, mock_project_service, mock_template_service):
        ...
        if len(call_args[0]) > 2:  # If profile is positional
            ...
            assert call_args[1].get('profile') == profile or call_args.kwargs.get('profile') == profile
    ...
    def test_get_project_info_not_found(self, cli_service, mock_project_service):
        ...
        mock_project_service.get_project_config.side_effect = FileNotFoundError("Project not found")
    ...
    def test_generate_project_template_success(self, cli_service, mock_template_service):
        ...
        # Assert
        assert result is True
        mock_template_service.generate_template.assert_called_once_with(template_name, output_path)
    
    def test_generate_project_template_invalid_template(self, cli_service, mock_template_service):
        """Test template generation with invalid template."""
        # Arrange
        mock_template_service.generate_template.side_effect = ValueError("Invalid template")
...
class TestCLIApplicationServiceIntegration:
    ...
    def test_error_handling_in_workflow(self, realistic_services):
        ...
        realistic_services["project"].project_exists.return_value = True  # Project exists
